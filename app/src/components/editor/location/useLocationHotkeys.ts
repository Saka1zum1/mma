...
import { singletonPano, getPanorama, clearSingletonPano, capturePano } from "@/lib/sv/panoSingleton";
...
export function useLocationHotkeys(deps: LocationHotkeyDeps) {
	...
	const buildDrop = async (): Promise<Location | null> => {
		...
		const live = capturePano(getPano());
		if (!live) return null;
		const liveProvider = isBaiduPanoId(live.panoId)
			? "baidu"
			: isTencentPanoId(live.panoId)
				? "tencent"
				: getLocationProvider(location);
		const tags = (await createTags(pendingTags)).map((tag) => tag.id);
		return dropLocation(
			{ ...location, provider: liveProvider },
			live,
			selectedPanoId ?? live.panoId ?? location.panoId,
			tags,
		);
	};
