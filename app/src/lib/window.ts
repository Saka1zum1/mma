...
/** Fork-facing wrappers kept so map-list / boot / updater call sites stay stable. */
export async function openMapWindow(id: string, name: string): Promise<void> {
	return openWindow({ type: "editor", mapId: id }, name);
}

export async function openMapWindowIds(): Promise<string[]> {
	return (await openWindows("editor")).map((w) => w.mapId);
}

export async function closeAllMapWindows(): Promise<void> {
	return closeWindows("editor");
}
