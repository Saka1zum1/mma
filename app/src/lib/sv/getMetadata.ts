...
/** Fork alt-provider rows (`baidu`/`tencent`/`yandex`/`apple`) must not hit Google RPCs. */
export function isGoogleImagery(row: { provider?: string | null }): boolean {
	const p = row.provider;
	return p == null || p === "" || p === "google";
}
