/**
 * Yandex enrichment — patches Google-semantic extra fields from panoramas API.
 */
...
export function registerYandexEnrichment(): void {
	registerEnrichmentProvider({
		id: "yandexMeta",
		label: "Yandex Street View metadata",
		fieldDefs: {},
		async enrich(locations, enrichFields, ctx?: EnrichCtx) {
			...
			const targets = locations.filter(
				(l: Location) => getLocationProvider(l) === "yandex",
			);
			...
			for (const loc of targets) {
				...
				try {
					...
					const meta = savedId
						? await fetchYandexMeta(savedId)
						: await resolveYandexNear(loc.lat, loc.lng);
					if (!meta) continue;
					const full = buildYandexExtra(meta);
					...
					log.warn("[yandex] enrich failed", loc.id, e);
				}
			}
			return patches;
		},
