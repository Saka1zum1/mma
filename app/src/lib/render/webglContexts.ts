...
/** Drop contexts whose canvas has left the document so a leaked overlay cannot
 *  exhaust the budget and take the next legitimate context with it. */
function evictDetached() {
	for (const canvas of [...live.keys()]) {
		if (!canvas.isConnected) lose(canvas);
	}
}
...
prototype.getContext = function (this: HTMLCanvasElement, type: string, attrs?: unknown) {
	...
	if (!live.has(this) && live.size >= WEBGL_CONTEXT_BUDGET) {
		log.error(
			`[webgl] context budget of ${WEBGL_CONTEXT_BUDGET} exceeded; evicting the oldest context:\n${describeLive()}`,
		);
		evictOldest();
	}
