...
pub fn patch_batch(
    ...
    let providers = col_provider(batch);
    ...
    for (&i, p) in &hits {
        ...
        touched[COL_PROVIDER] |= if providers.is_null(i) {
            p.provider.is_some()
        } else {
            p.provider.as_deref() != Some(providers.value(i))
        };
        ...
        .map(|(ci, col)| {
            ...
            match ci {
                ...
                COL_TAGS => {
                    ...
                    for i in 0..n {
                        match hits.get(&i) {
                            Some(patch) => b.values().append_slice(&patch.tags),
                            None => b.values().append_slice(
                                &old_values[offsets[i] as usize..offsets[i + 1] as usize],
                            ),
                        }
                        b.append(true);
                    }
