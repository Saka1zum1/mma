...
pub struct LocView<'a> {
    ...
    providers: Option<&'a StringArray>,
...
impl<'a> LocView<'a> {
    pub fn new(
        ...
        use crate::arrow_bridge::{
            ...
            col_modified_at, col_pano_id, col_pitch, col_provider, col_tags, col_zoom,
        };
        ...
        let providers = batch.map(col_provider);
...
builtin_fields! {
    ...
    "provider", "Provider", ExtraFieldType::String, None, None,
        |l| l.provider.as_deref().map(|p| serde_json::json!(p)),
        |v, i| v.providers.and_then(|c| {
            (!c.is_null(i)).then(|| serde_json::json!(c.value(i)))
        });
...
/// One grouping projection a field type may be partitioned by.
#[derive(Clone, Serialize, specta::Type)]
#[serde(rename_all = "camelCase")]
