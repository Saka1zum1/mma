//! A pasted Google Maps URL -> the location it names. Short links resolve through
//! [`crate::short_link_target`] first; the expanded grammar covers the
//! `/maps/@...!1s<key>!2e<frontend>` path form, `map_action=pano` viewpoints, the legacy
//! `layer=c&cbll=` form, and Arts & Culture street views.
//!
//! Alt-provider URLs (Baidu / Tencent / Yandex / Lookmap) stay in JS.
...
fn google_provider() -> Option<String> {
    Some("google".into())
}
...
fn put_varint_field(buf: &mut Vec<u8>, field: u32, v: u64) {
    put_varint(buf, (u64::from(field) << 3) | 0);
    put_varint(buf, v);
}
...
/// Protobuf `ImageKey` `[frontend, id]` -> pano ID string. Mirrors JS `imageKeyToPanoId`.
fn from_image_key(frontend: i32, id: &str) -> String {
    if id.is_empty() {
        return String::new();
    }
    if frontend == 0 || frontend == 2 {
        return id.to_string();
    }
    if frontend == 3 {
        return format!("F:{id}");
    ...
    let mut buf = Vec::new();
    put_varint_field(&mut buf, 1, frontend as u64);
    put_str(&mut buf, 2, id);
...
pub(crate) fn parse(input: &str) -> Option<ParsedLocation> {
    ...
    if let Some(mapsapp) = mapsapp {
        ...
        if let Some(id) = id {
            let target = crate::short_link_target(&id, mapsapp).ok()??;
