...
impl ProvidersSettings {
    ...
    pub fn migrate_legacy_alt_basemap(&mut self) {
        let mut alt = self.alt_basemap_settings.clone().unwrap_or_default();
...
fn row_to_map_meta(row: &rusqlite::Row<'_>) -> Result<MapMeta, rusqlite::Error> {
    ...
    Ok(MapMeta {
        ...
        settings: {
            let mut settings: MapSettings = serde_json::from_str(&settings_str).unwrap_or_default();
...
pub async fn store_update_map_meta(
    ...
    let new_fields = patch.extra.as_ref().and_then(|extra| extra.fields.clone());
    let row_id = id.clone();
    ...
    if let Ok(store) = manager.store_for_map(&id) {
        if let Some(old_fields) = old_extra.and_then(|extra| extra.fields) {
