...
fn fetch_streamed(
    ...
    drive(async {
        let mut pending: futures::stream::FuturesUnordered<_> = reqs
            .iter()
            .enumerate()
...
fn fetch_all(
    ...
    out.into_iter()
        .map(|r| r.expect("every request answers"))
        .collect()
}
...
pub(crate) fn run_provider(ctx: &RunCtx, decl: &ProviderDecl) -> AppResult<()> {
    ...
    let outcome = std::thread::scope(|s| {
        for _ in 0..instances {
            ...
            s.spawn(move || {
                let _inst = run.instance();
                run_instance(ctx, decl, budget, prog, config, batch_rx, &out_tx);
            });
        }
...
fn field_present(loc: &Location, field: &str) -> bool {
    match field {
        ...
        "provider" => loc.provider.as_ref().is_some_and(|p| !p.is_empty()),
        _ => loc
            .extra
            .as_ref()
...
pub async fn procedure_run(
    ...
    let map_id = state.lock()?.map_id_for_window(webview.label())?;
