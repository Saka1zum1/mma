...
use crate::selections::{compare_filter, FilterOp};
...
impl Display for ExprError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExprError::MissingLeftOperand => f.write_str("Expected a value before the comparison"),
...
pub enum Expr {
    ...
    Str(String),
    Field(String),
    /// `has(field)`: 1 when the resolver returns a value, 0 otherwise. Never skips
    /// the row, which is the whole point -- absence is an answer, not a failure.
    Has(String),
    Neg(Box<Expr>),
