...
fn a_pano_viewpoint_url_carries_the_whole_camera() {
    ...
    assert_eq!(p.provider.as_deref(), Some("google"));
}
...
#[test]
fn alt_provider_hosts_are_left_to_js() {
    assert_eq!(
        parse(
            "https://map.baidu.com/search/x/@13517338.48,3312052.08,13z#panoid=09025100011602251440355768N"
        ),
        None
    );
    assert_eq!(parse("https://lookmap.skzk.dev/#c=5/41.36/-8.39"), None);
}
