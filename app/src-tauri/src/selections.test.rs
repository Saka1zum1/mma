...
#[test]
fn provider_is_filterable_and_projectable_across_storage_layers() {
    let mut google = loc(1, 0.0, 0.0);
    google.provider = Some("google".into());
    let mut patched_base = loc(2, 0.0, 0.0);
    patched_base.provider = Some("google".into());
    let mut apple = patched_base.clone();
    apple.provider = Some("apple".into());
    let mut tencent = loc(3, 0.0, 0.0);
    tencent.provider = Some("tencent".into());
    let fx = Fx::base(&[google, patched_base])
        .with_patch(2, apple)
        .with_adds(vec![tencent]);
    ...
    assert_eq!(
        resolve(&fx.view(), &selector).iter().collect::<Vec<_>>(),
        vec![2]
    );
    assert_eq!(
        distinct_values(&fx.view(), "provider", None),
        vec!["apple", "google", "tencent"]
    );
    let mut counts = count_by(&fx.view(), "provider", &KeySpec::Value, None);
    counts.sort();
    assert_eq!(
        counts,
        vec![
