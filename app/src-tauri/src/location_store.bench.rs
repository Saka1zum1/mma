//! Bench support: deterministic fixtures plus a [`BenchApp`] harness that drives
//! the real store mutation paths. Compiled only under `--features bench` and
//! re-exported as `app_lib::bench_api`; inert in every normal build.
//!
...
//! visibility. Command-level benches go through [`BenchApp`] — the same bodies
//! the IPC commands run, minus the `Webview` argument the fork commands take.

use super::*;
use crate::arrow_bridge::col_id;
use crate::types::{LocationFlags, RawExtra};
...
// ---------------------------------------------------------------------------
// BenchApp: command bodies without a Tauri Webview
// ---------------------------------------------------------------------------

/// In-process store harness. The fork's IPC commands take a `Webview`; benches
/// drive the same mutation bodies through a managed [`StoreState`] keyed `"bench"`.
pub struct BenchApp {
    state: StoreState,
}
...
impl BenchApp {
    ...
    fn with_store<T>(&self, f: impl FnOnce(&mut Store) -> T) -> T {
        let mut mgr = self.state.lock().expect("store lock");
        f(mgr.stores.get_mut("bench").expect("bench store"))
    }

    pub fn add_locations(&self, mut locations: Vec<Location>) -> MutationResult {
    ...
    /// Full render via the same buffer builder the command wraps; temp-file write included.
    pub fn fill_render(&self) -> String {
        let buf = self.with_store(|store| {
            let req = render_request();
            store.render.arrow_style = false;
            build_cell_render_buffers(store, &req)
        });
        let path = std::env::temp_dir().join("mma_bench_render.bin");
        std::fs::write(&path, &buf).expect("write render");
