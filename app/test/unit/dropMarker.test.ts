...
import { createLocation, dropLocation, LocationFlag, type PanoCapture } from "@/types";
...
describe("dropLocation", () => {
	...
	it("carries the source's flags, provider, and the tags it is handed", () => {
		const drop = dropLocation(source, live, live.panoId, [1, 2]);
		expect(drop.flags).toBe(LocationFlag.LoadAsPanoId);
		expect(drop.provider).toBe(source.provider);
