...
describe("Import â€?escaped quotes in extra fields", () => {
...
describe("Import â€?panoId with special characters", () => {
	useMap("E2E PanoId Special");

	it("preserves panoId containing unicode", async () => {
		const result = await withApi(async (api) => {
			const json = JSON.stringify({
				customCoordinates: [{ lat: 10, lng: 20, panoId: "CAoSLEFGMVFpcE5Ã©abc" }],
			});
		...
		expect(result.panoId).toBe("CAoSLEFGMVFpcE5Ã©abc");
...
describe("Import â€?countryCode/stateCode at top level", () => {
...
describe("Import â€?nested objects in extra", () => {
...
describe("Import â€?empty customCoordinates", () => {
...
describe("Import â€?export/reimport tag round-trip", () => {
