...
export function normalizeStops(raw: unknown): GradientStop[] {
	...
	return (raw as GradientStop[])
		...
		.map((s) => ({ color: [...s.color] as RGB, pos: clamp01(Number(s.pos) || 0) }))
...
export function resolveGradient(id: string, customs: HeatmapGradient[]): HeatmapGradient {
	return (
...
export function moveStop(
	...
	const moved: GradientStop = { color: [...target.color] as RGB, pos: clamp01(pos) };
...
export function setStopColor(stops: GradientStop[], index: number, color: RGB): GradientStop[] {
